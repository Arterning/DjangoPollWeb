...
/**
read n chars from fd to buffer
*/
ssize_t readLine(int fd, void *buffer, size_t n)
{
    ...
    totalRead = 0;
    for (;;) {
        alreadyRead = receive_data(fd, &ch, 1);
        ...
        } else if (alreadyRead == 0) {      /* EOF */
            if (totalRead == 0)           /* No bytes read; return 0 */
                return 0;
            else                        /* Some bytes read; add '\0' */
                break;

        } else {                       
            if (totalRead < n - 1) {      /* Discard > (n - 1) bytes */
                totalRead++;
                *buf++ = ch;
            }
...
/**
read http header info from fd to buffer
*/
...
//返回proxy的运行基本信息
void print_proxy_info(int sock, char* header) {
...
/* 获取运行的基本信息输出到指定的缓冲区output */
void get_info(char* output)
{
    int pos = 0;
    char line_buffer[512];
    sprintf(line_buffer,"======= proxy (v0.1) ========\n");
    ...
    if(strlen(remote_host) == 0) /* 未指定远端主机名称从http 请求 HOST 字段中获取 */
    {
        ...
        } else 
        {
            ...
            if(strstr(header_buffer,"GET /proxy") >0 ) 
            {
                ...
                //返回proxy的运行基本信息
                print_proxy_info(client_sock,header_buffer);
...
void forward_data(int source_sock, int destination_sock) {
    ...
    // core logic is receive data and send
